...
import { GET_TODO, GET_FILTERED_TODO } from 'graphql/query';
import { ADD_TODO, UPDATE_TODO, DELETE_TODO } from 'graphql/mutation';

function TodoList() {
	const { data, loading } = useQuery(GET_TODO);
	const [getFiltered, { data: filteredData, loading: filteredLoading }] = useLazyQuery(GET_FILTERED_TODO);
	const [addTodo, { error: addError }] = useMutation(ADD_TODO, { refetchQueries: [{ query: GET_TODO }] });
	const [updateTodo, { error: updateError }] = useMutation(UPDATE_TODO, { refetchQueries: [{ query: GET_TODO }] });
	const [deleteTodo, { error: deleteError }] = useMutation(DELETE_TODO, { refetchQueries: [{ query: GET_TODO }] });
	...
	useEffect(() => {
		if (!filteredLoading && filteredData) {
			return setList([...filteredData.todolist]);
		}
	...
	const onFilterHandler = (e) => {
		...
		if (searchQuery === '') {
			...
			getFiltered({ variables: { id: searchQuery } });
		}
		setSearchQuery('');
	};
	...
	const onAddHandler = (e) => {
		...
		addTodo({ variables: { title: addQuery } });
		setAddQuery('');
	};
	...
	const onClickItem = (e, id) => {
		...
		updateTodo({ variables: { id: taskID, isComplete: !task.isComplete } });
	};
	...
	const onDeleteItem = (e, id) => {
		...
		let confimation = window.confirm('test');
		if (confimation) {
	...
	return (
		<>
			<div className='container'>
				...
				<form className='js-form' onSubmit={onFilterHandler}>
					<input
				...
				{loading || filteredLoading ? (
					...
					<>
						<ul className='todo-list js-todo-list'>
							{list.map((v, i) => (
								<Todo
									key={i}
									id={v.id}
									onClickItem={(e) => onClickItem(e, v.id)}
									onDeleteItem={(e) => onDeleteItem(e, v.id)}
									title={`(${v.id}) ${v.title}`}
						...
						<form className='js-form' onSubmit={onAddHandler}>
							<input
								onChange={onAddInput}
								value={addQuery}
								autoFocus
							...
							<div>
								<button type='submit' className='form-filter form-button'>
									Add task
								</button>
							</div>
						</form>
